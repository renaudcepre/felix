...
def has(ents: list[dict], frag: str) -> bool:
    ...
    return any(
        ...
        for e in ents
        if e.get("entity_type") != "evenement"
    )
...
async def main() -> int:
    ...
    async with lifespan(app):
        ...
        async with httpx.AsyncClient(
            ...
            checks.append(
                (
                    "tour 1 crée Ottra et Vels",
                    has(ents, "ottra") and has(ents, "vels"),
                    f"{sorted(names_of(ents))}",
                )
            )
            card_ids = [
                c.get("entity_id") for c in cards if c.get("title") == "Entité créée"
            ]
            checks.append(
                (
                    "les cartes « Entité créée » portent entity_id",
                    bool(card_ids) and all(card_ids),
                    f"{card_ids}",
                )
            )
            ...
            checks.append(
                (
                    "Ottra absente de la bible après DELETE",
                    "ottra" not in names_of(ents),
                    f"{sorted(names_of(ents))}",
                )
            )
            async with driver.session() as session:
            ...
            checks.append(
                (
                    "la suppression laisse un tombstone :UserEdit",
                    bool(rec) and rec["n"] >= 1,
                    f"{rec['n'] if rec else 0} tombstone(s)",
                )
            )
            ...
            checks.append(
                (
                    "le tour suivant ne RECRÉE pas Ottra (anti-résurrection)",
                    not has(ents, "ottra"),
                    f"{sorted(names_of(ents))}",
                )
            )
            checks.append(
                (
                    "le tour suivant écrit quand même (Doran capté)",
                    has_any(ents, "doran"),
                    f"{sorted(names_of(ents))}",
                )
            )
            ...
            checks.append(
                (
                    "PATCH rename Vels → Velsgarde → 200",
                    resp.status_code == 200,
                    f"HTTP {resp.status_code}",
                )
            )
            ...
            checks.append(
                (
                    "l'id a migré, type conservé (même effet que rename_entity)",
            ...
            checks.append(
                (
                    "l'ancien nom ne ressuscite pas la fiche renommée",
                    before["id"] not in ids and "velsgarde" in ids,
                    f"{sorted(ids)}",
                )
            )
