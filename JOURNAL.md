...
**État (2026-06-09)** : **le front est passé 100 % schemaless** (section juste en dessous). `/atelier → /chat`, nouvelles pages `/entities` (liste par type + fiche générique props/relations/chronologie), **tout le legacy `:Character` supprimé** (front + API + repos + ingest + agent + cli, ~85 fichiers). Surface conservée vérifiée end-to-end (imports, ruff sans import mort, curl entities, route SSE atelier vivante, typecheck/lint front verts). Moteur `core/`/`atelier/` **non touché**. Les **quick-wins moteur (1)(2)(3) ci-dessous restent en attente**.
