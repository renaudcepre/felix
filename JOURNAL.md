...
## ▶ HANDOFF (reprendre ici) — v1 validité d'état + 2 quick-wins — 2026-06-08

**État** : modèle événementiel **v0 fait** (section ci-dessous), suite atelier verte hors transients réseau (backend Mistral très instable ce jour : `BrokenResourceError`/503, un run de 34 min). Reste, par ordre :

**(1) v1 du modèle événementiel — dériver la validité des états depuis les events.**
- v0 stocke les events ordonnés (`ordre`+`NEXT`). v1 = répondre « P vrai au beat N ? » = défaut(prop/relation) ± events jusqu'à N, **sans tamponner** la validité sur les états (on DÉRIVE, cf. calcul d'événements / fluents). Débloque le **checker « mort puis agit »** : un event de mort d'`ordre` k rend contradictoire tout event `INVOLVES` le mort d'ordre > k. Réf. `docs/vision_produit_copilote_scenario.md`. Eval-first (cf. [[feedback_tdd_evals]]).
