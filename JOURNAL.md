...
## #68 — sortie du tapis roulant : noyau structurel dur + narratif en verbe VERBATIM (LIE_A) — 2026-06-11

**Le design de #68 implémenté** (bascule demandée sans attendre l'instrumentation des liens tus). La sémantique narrative change de couche : le vocab dur ne garde que le **noyau STRUCTUREL** que le code calcule ou affiche structurellement (`LOCATED_AT`/`MEMBER_OF`/`PART_OF` en `RelationSpec`, domaine/portée conservés ; `INVOLVES`/`NEXT` restent en code) ; **tout lien narratif passe par UN type générique `LIE_A` + le verbe verbatim de l'auteur en prop** (`Tessa —[:REL {rel_type:'LIE_A', verbe:'était la maîtresse de'}]→ Hadrin`). Zéro trou par construction, zéro traduction qui ment, zéro soupe — la liberté va dans la DONNÉE, pas dans le schéma. La sortie légale de c7e49a7 devient une REDIRECTION : le refus hors-vocab pointe vers `LIE_A {verbe}` (le droit de se taire reste si le texte ne pose pas le lien).

**Les « à trancher » de l'issue, tranchés.** (a) **Dédup : MERGE sur (paire, rel_type, slug du verbe)** — deux liens différents entre la même paire (aime ET commande) = deux arêtes ; la même paraphrase exacte n'en crée qu'une ; `describe_schema` liste les verbes déjà posés pour inciter à la réutilisation. (b) **Cohabitation, pas de migration** : les anciens types (LOVES…) restent lisibles tels quels (`rel_label` rend le verbe pour LIE_A, le type sinon) — inventer rétroactivement un verbe depuis un type serait précisément la traduction qui ment qu'on tue. (c) **Evals : `relation_typed` conservé pour le structurel, nouvel evaluator `relation_verbe`** (LIE_A entre a et b + verbe contenant un terme attendu) — les 3 sondes #48 deviennent `verbe_maitresse`/`verbe_ordonne`/`verbe_transporte` ; `CANONICAL_RELS` resserré à {located_at, member_of, part_of, lie_a, involves, next} (un ancien type narratif compte désormais comme dérive). (d) **#53 (Cypher typé) de facto re-scopé au noyau structurel.** (e) Direction : chantier à part, inchangé.

**Câblage.** `Profile.narrative_rel` (+ rendus prompt/schema-hint), `validate_relation(verbe=…)` (verbe REQUIS, pas de boucle, AUCUN domaine/portée sur le narratif), `add_relation(verbe=…)` (param explicite — pas une prop enfouie — + clé de MERGE narrative), `NARRATIVE_REL`/`REL_RESERVED_KEYS`/`rel_label` dans `core/graph.py`, `merge_entity_into` en 2 passes (la fusion d'entités préserve les arêtes LIE_A multiples), `delete_relation(verbe_slug=…)` + route DELETE + front (la même paire porte plusieurs arêtes — sans le slug, supprimer « aime » emporterait « commande »), `neighborhood` (le juge lit le verbe EXACT), fiche/cartes affichent les mots de l'auteur (badge = verbe), prompts relieur réécrits (les 2 couches). Thème papier : l'auteur ne voit jamais « LIE_A ».

**Mesuré.** Unit **83/83** (suite RelationTyping réécrite, 14 cas : rejets structurels d'Alger conservés, verbe requis, hors-vocab redirigé, LOVES rejeté, tolérance schemaless intacte). Sondes verbe **3/3 vertes** en LLM réel — le relieur pose spontanément `LIE_A {verbe:'ordonne à'}`, plus précis que l'ex-COMMANDS. Suite atelier complète : **17/22 au premier passage, les 5 échecs requalifiés VERTS en re-run isolé** (soirée API à transients, encore : un cas à 288 s absorbé par le timeout #55, `request_limit` de 50 mangé par les retries → event_chrono/event_no_bleed tués par quota pas par verdict — le backoff #67 reste à faire ; `etat_interne_prop` = flakiness #64 connue, vert en isolation ; `check_death_then_act` = transient de validation juge). **`rel_vocab_coverage` 1.00 et `rel_recall` 1.00 sur multi_beats** (rel_type_count 5 : structurel + lie_a + machinerie events). Baseline comparée à chaud : `vocab_commands` vert sur main au même moment — la flakiness du cas « ordonne » est pré-existante, pas une régression du design. Ruff (0 nouvelle erreur, 2 noqa justifiés), eslint, `nuxt typecheck` verts.

**Reste.** Fermer/re-scoper #53 et fermer #68 (actions GitHub) ; la direction des arêtes (chantier existant) ; le backoff transients (#67) qui aurait évité toute la requalification manuelle de ce soir.

## Persistance du chat + timeout client (#55) + revue d'issues (#63/#64) — 2026-06-10

**Persistance de la conversation (front).** Retour utilisateur : quitter `/chat` et revenir vidait la conversation. Cause : `useAtelier()` recréait des refs neuves à chaque appel. Fix : état **singleton au niveau module** + **localStorage** (`felix.atelier.conversation.v1`) — fil affiché ET `messageHistory` threadé au LLM survivent navigation + F5. Watcher de persistance dans un `effectScope(true)` **détaché** (sinon tué au démontage de la 1ʳᵉ page → la persistance des édits #61 s'arrêtait après une navigation). Bouton « Nouvelle conversation » (double-clic de confirmation, pattern 🗑 #61). Assumé : rustine local-first mono-utilisateur — la cible est la conversation **en graphe** (#63).

**Timeout client LLM (#55, implémenté main).** `build_model` passe un `httpx.AsyncClient(timeout=Timeout(90, connect=10))` aux deux providers — un appel Mistral gelé coûtait ~900 s de timeout SDK (3 occurrences mesurées Logfire). Smoke OK sur les deux branches (Mistral + OpenAI-compatible), ruff/pyright verts. Limite notée : client créé par `build_model` sans close — OK tant que les agents se construisent au démarrage ; à mutualiser si `build_model` devient par-requête.

**Revue d'issues.** Ménage legacy : #9 (ChromaDB, `loader.py` n'existe plus) et #12 (logging → Logfire couvre) **fermées** ; #42 re-scopée P3 (felix-export supprimé ; le rollback ira au modèle événementiel) ; #22 annotée (stack Neo4j, plus SQLite/Chroma). Quick-fix identifiés : #55 ✅, #48 (vocab), #58 (résolution hors events sur update — `find_non_event` existe déjà pour `add_relation`), #51 (multi-run).

**#63 — conversation en Neo4j (idéation).** Nœuds `:Message` (méta-nœuds hors `:GenEntity`) + provenance (`:PRODUCED`/`:MENTIONS`) puis **repêchage par pertinence** : le contexte long ne se gère plus par récence (fenêtrage) mais par liaison au focus courant via le graphe. Contrainte de conception posée par l'utilisateur : **PULL uniquement, jamais d'auto-injection** — vider la conversation pour reprendre du recul est une FONCTION à protéger ; l'archive en graphe rend justement le reset gratuit (recul sans perte). Briques 1+2 à grouper avec Carnet/#60 ; brique 3 différée jusqu'à un vrai projet long (eval-first le jour venu).

**#58 — garde anti-événement sur `update_entity` (fix DÉLÉGUÉ, test du pattern sous-agent Sonnet).** Premier essai « brief complet → sous-agent Sonnet exécute » : fix correct (garde `check_update_target` pure après `find_node` — équivalent au plan `find_non_event` grâce au tie-break non-événement de `find_node`, avec un message guidant MEILLEUR : « est un événement, pas une entité » quand seul un event matche), test anti-leakage structurel (noms neufs Brulard/Gretchen/Orvaine + grep des sources), rapport honnête (le `×` RUF003 signalé pré-existant : vérifié vrai, corrigé au passage). Réserve : le « rouge » prouvé était un ImportError, pas une reproduction comportementale du bug — l'esprit eval-first demande un test qui échoue sur le COMPORTEMENT actuel. Verdict pattern : OK pour les quick-fix bien spécifiés, revue obligatoire ; ~76k tokens Sonnet, ~9 min, 42 appels d'outils.

**#51 — `bapteme_differe` en MULTI-RUN (2ᵉ délégation Sonnet, pattern confirmé).** Le cas flaky (vert/rouge/vert) sort du `ForEach` et devient une eval standalone : `run_atelier_multirun_case` (générique : N passes EN SÉRIE, chaque pass sur graphe wipé, somme des tokens) + evaluator `multirun_majority(2, 3)` lisant `multirun_passes` sur le résultat. Preuve comportementale en live : pass 3 a flaké (« Un mage noir » en doublon d'Alikazeth — exactement le bug) et le seuil l'a absorbé → 2/3 = vert. Coût : ~3× le mono-run (~18 appels, $0.015/run). Le `Retry(times=N)` de protest examiné et écarté à raison (résilience sur exception, sémantique opposée au vote majoritaire). Dette assumée : `_bapteme_differe_check` réplique inline `graph_char_count`+`entity_unique` (dérive possible, documentée). Plomberie réutilisable pour `roue_de_sang`/`event_chrono` si besoin. **Découverte au passage** : `tools/view_history.py` lit `evals/results/history.jsonl` (legacy) alors que protest 0.2.0 écrit `.protest/history.jsonl` avec un autre schéma → le suivi d'historique est silencieusement aveugle (issue posée).

**#48 + #64 — vocab relations, sortie légale, garde anti-sur-entification (eval-first).** Quatre sondes écrites et vérifiées ROUGES d'abord : 3 trous de vocab (`vocab_loves`/`vocab_commands`/`vocab_transports`, nouvel evaluator `relation_typed` qui vérifie le TYPE — un KNOWS forcé ne passe plus pour un succès) + `etat_interne_prop` (#64). Leçon de reproduction : le mono-tour « Ossian souffre d'une dépression » était VERT — c'est le tour de CORRECTION (« de synthèse je voulais dire ») qui déclenche la cascade (entité `dépression [état]` auto-étiquetée + doublon + KNOWS forcé + event chroniqué) ; la sonde joue les 2 beats → 2/2 rouges. **Changements** : (a) vocab : `COMMANDS`/`TRANSPORTS`/`LOVES` ajoutés (`HELPS` écarté — ALLIED_WITH couvre « aide ») ; (b) SORTIE LÉGALE au trou de vocab : message de refus de `validate_relation` + prompts relieur (les 2 couches) + docstring add_relation — « si aucun type ne colle, ce lien ne s'écrit pas », la consigne « rabats-toi sur le plus proche » (HANDOFF (1)) est INVERSÉE, contredite par la session Lancelot ; (c) consigne « état interne = prop » dans SYSTEM_PROMPT (exemple prompt-only Edran/fièvre grise) — INSUFFISANTE seule sur le tour de correction → (d) **garde symbolique à add_entity** (types `etat`/`maladie`/`sentiment`/`emotion`/`humeur` refusés avec message guidant, pattern garde `evenement`) ; avec la garde : 2/2 verts. **Non-régression** : suite complète 18/18 verts sur cas exécutés (4 tués par transients API, tous requalifiés : roue_de_sang ✓, event_chrono ✓, vocab_commands ✓ après découplage de sa sonde — le `graph_char_count` couplé flakait sur « capitaine Ressac »/« Ressac », un chantier résolution à part —, act_then_death = variance JUGE pré-existante, graphe parfait mais lecture fautive → candidat multi-run, #67). Unit 82/82. **API Mistral en vrac toute la soirée** (500/504/400, suite à 20 min) → issue #67 : backoff transients dans run_atelier_case (pattern with_backoff existant) + multi-run act_then_death ; le timeout #55 a fait son boulot (gels morts en 90 s).

**#68 — le vocab fermé ne passe pas à l'échelle (design, suite immédiate de #48).** Réaction auteur à chaud devant les 3 types ajoutés : « on va ajouter tous les verbes ? » — l'énumération ne termine pas. Diagnostic posé : la sémantique narrative est dans la mauvaise couche (le code ne CALCULE que le structurel — INVOLVES/NEXT/LOCATED_AT/PART_OF/MEMBER_OF ; LOVES/FIGHTS ne sont qu'affichés et lus en texte par le juge). Design cible dans l'issue : **noyau structurel fermé (vocab dur conservé — il a tué la soupe d'Alger, on ne rouvre PAS le nommage libre) + narratif en \`LIE_A {verbe: verbatim}\`** — la liberté va dans la DONNÉE, pas dans le schéma, symétrique de la philosophie entités. Le verbe de l'auteur est plus fidèle qu'un type traduit (bloc-notes, pas traducteur). Critère de bascule AVANT d'implémenter : mesurer la fréquence des liens « tus » par le relieur (possibles depuis la sortie légale de c7e49a7) sur les prochains dogfoods. Impact : #53 à re-scoper (le Cypher typé ne vaudra jamais que pour le noyau).

**#66 + #65 — nettoyage et outillage DÉLÉGUÉS EN PARALLÈLE (3ᵉ/4ᵉ délégation Sonnet).** Deux sous-agents simultanés sur zones disjointes, coordination briefée (le chantier #66 déplaçait `typer`/`rich` vers le groupe dev pendant que #65 réparait l'outil qui les utilise — chacun averti de l'autre). **#66** (revue dead-code faite PAR l'utilisateur, exécution déléguée avec re-vérification par item) : shim `atelier/models.py`, settings `segmenter_*` morts, per-file-ignores `scripts/**`, police Neuropol (css + 2 binaires), `evals/fixtures/` orphelins (21 fichiers), `REVIEW.md`/`JOURNAL_UNIT_PROTEST.md`, `typer`/`rich`→dev + `torch` transitif, cruft non tracké (`exports/`, dirs vides). Tout re-vérifié avant suppression, 9/9 faits, suite 82/82 + lint/typecheck/build front verts. **#65** : `view_history.py` réécrit — défaut `.protest/history.jsonl`, détection de format PAR LIGNE (protest 0.2.0 `suites.<nom>.cases` vs legacy `cases`), les deux formats lisibles, skill check-history mise à jour ; le flake de `bapteme_differe` est maintenant VISIBLE dans l'historique (FAIL 05h31 → PASS 05h36). Accroc de délégation noté : l'agent #65 a ajouté une section HANDOFF au JOURNAL hors de son périmètre (retirée — le récit reste tenu ici). Pattern confirmé : briefs étanches + revue centrale, ~126k tokens Sonnet les deux chantiers.

**#64 — retour front « Lancelot WITNESSES dépression ».** L'extracteur **entifie une maladie** (type libre `maladie`) → le relieur force `WITNESSES` puis `KNOWS` (le « dernier recours » du vocab) faute de type qui colle ; la garde domaine/portée est **aveugle sur type d'entité inconnu** (règle 3 tolérante de `validate_relation`) ; la correction « je voulais dire » crée un **doublon** au lieu d'un `rename_entity`. Diagnostic : la racine n'est PAS le vocab fermé mais (a) la **sur-entification** (état interne = prop du perso, pas un nœud — même famille que #46/#47) et (b) l'absence de **sortie légale** au trou de vocab (le silence est impossible → mensonge en-vocabulaire). #48 amendé : la consigne « rabats-toi sur le plus proche » est contredite par cette session — l'inverser en « rien ne colle → n'émets rien, pose en prop ».

## Maître : interviewer → BLOC-NOTES (retour utilisateur) — 2026-06-10

**Le problème (utilisateur, direct).** « Il faut arrêter de lui dire de proposer une suite, c'est horrible, c'est pas ce qu'un auteur veut — on veut un bloc-notes. » Le maître « réagis en une phrase puis relance avec UNE question » questionnait à CHAQUE tour (« Quel est le défi ? », « Qui y vit ? », « Où est son échoppe ? ») — l'A/B le chiffre : **24/24 relance**. Et le même réflexe « avoir l'air engagé » faisait CONFABULER du concret sur base vide (Talou « aux doigts crochus, barbe grisonnante » — jamais donné par l'auteur ; sur base peuplée il restait abstrait « passé mystérieux »). Le bug castor #62 (« comment s'appelle-t-il ? ») n'était qu'un **symptôme** de ce réflexe.

**La bascule.** `MASTER_PERSONA`/`MASTER_SYSTEM_PROMPT` réécrits « interviewer » → « BLOC-NOTES » : RÉAGIR en une phrase courte à l'histoire (accrochée au concret, sans formule passe-partout), NE PAS questionner (sauf si l'auteur cale/sollicite), NE RIEN inventer, ne pas dire « noté », ne pas deviner la bible. Voix **« réagit sobrement, jamais de question »** choisie par l'utilisateur (vs « confirme en un mot » / « silencieux », proposées en preview). Câblage : AUCUN — seul le prompt du maître change ; l'`ATELIER_PERSONA` qui questionne encore est le **pass-1 extracteur** (`stream_text=False`, jamais montré à l'auteur), hors sujet.

**Mesuré** (`just ab-blocnotes`, `evals/atelier/master_blocnotes_ab.py` — A/B de PERSONA, small, base vide). Deux métriques : RELANCE = « ? » présent (déterministe) ; CONFABULATION = juge structuré « affirme un fait concret non donné ? ». **interviewer (gelé) 24/24 relance vs bloc-notes (live) 0/24 relance + 0/24 confabulation.** Le harness gèle l'ancien prompt en local et IMPORTE le prompt LIVE → il sert aussi de **garde-fou de régression** sur la voix shippée.

**Non-régression** : `just e2e-conductor` VERT. Le maître bloc-notes RÉPOND toujours aux questions (« qui est Nora ? » → « Nora est une journaliste qui enquête sur Castan à Port-Vendres » ; « résume » → « trois personnages et sept événements ») et ne pose plus de question sur les tours de contenu (« Castan a un homme de main, Veil » → « Un homme de main qui repère la journaliste, ça sent le danger »). 0 hallucination, recall 2/3 + 3/3, 0 réponse vide. Unit 75/75 (anti-leakage du nouveau prompt OK).

**Conséquence #62** : résolu À LA RACINE (le maître ne pose plus de relance réflexe). Le few-shot « ne redemande pas une info donnée » devient inutile : on ne redemande rien par défaut. Pointeurs : `src/felix/atelier/agent.py` (MASTER_PERSONA/MASTER_SYSTEM_PROMPT), `evals/atelier/master_blocnotes_ab.py`, [[project_master_blocnotes]].

## A/B tiering du MAÎTRE sur le bug « castor Hector » (#62/#49) : verdict NÉGATIF, tiering injustifié — 2026-06-10

**Question.** Le dogfood « castor Hector » (#62) : l'auteur écrit « Ok c'est l'histoire d'un castor, Hector », le maître répond « Un castor, c'est original ! **Comment s'appelle-t-il ?** » — il rate l'apposition et redemande un nom DÉJÀ donné. Seul bug qui vit uniquement dans le **jugement sémantique conversationnel** du maître (gate + extraction étaient justes). Le maître = 1 appel/tour + voix du produit → candidat n°1 au tiering (#49). **Monter le maître en tier règle-t-il ce bug ?**

**Harness** (`evals/atelier/master_ab.py`, `just ab-master`). On ISOLE la variable « modèle du maître » : threadé un `model` optionnel dans `create_core_agent`/`build_master_agent` (défaut prod inchangé — c'est aussi la plomberie #49) ; le maître est bâti EXACTEMENT comme en prod (persona+system_prompt+tools scénario), seul le tier change (small/medium/large). **Fidélité décisive** : on rejoue le tour DANS le cadre d'interview (on génère d'abord l'ouverture « Bonjour ! » → « Raconte-moi ton histoire », puis on threade le tour-piège par-dessus, comme la route SSE). **À froid (sans ce cadre), small ne reproduit JAMAIS le bug** — il lit l'apposition très bien ; c'est le cadre qui pourrait déclencher la relance réflexe. 5 tours-pièges (apposition « castor, Hector » verbatim + apposition+métier, subordonnée « qui s'appelle », « se nomme », double-info) + 1 contrôle (aucun nom donné → poser la question est LÉGITIME, garde-fou anti « le gros modèle gagne en se taisant »). Double jugement indépendant : **juge** structuré reason-first (medium ; juger « l'info est-elle dans le message ? » est trivial → libère le quota Large) **+ heuristique** mot-clé (« comment s'appelle… ») ; divergences imprimées. Backoff sur 429, **lecture seule → pas de wipe**. Réglages `FLX_AB_REPS` (défaut 6), `FLX_AB_MODELS`, `FLX_AB_JUDGE`.

**Résultat (REPS=10, 150 cellules-piège).**

| modèle | redondance (juge) | heuristique | contrôle |
|---|---|---|---|
| mistral-small-2506 | **0/50** | 0/50 | 0/10 |
| mistral-medium-latest | **0/50** | 9/50 | 0/10 |
| mistral-large-latest | **0/49** (1 glitch) | 0/49 | 0/10 |

**Le bug ne se reproduit sur AUCUN tier, small compris.** Sur l'input dogfood EXACT, small en 10/10 sort « Un castor, c'est original ! » (le *même* opener que le bug live !) puis enchaîne correctement « Quel est le problème d'Hector ? » — jamais « Comment s'appelle-t-il ? ». **Le « Comment s'appelle-t-il ? » live était un tirage de queue rare** (à temp 0.1 ; règle de trois sur 0/50 → taux borné < ~6 %). Les 9 hits heuristiques sur medium sont des **faux positifs validés par le juge** : medium demande « Quel est le nom du **déserteur** ? » — entité réellement non nommée (seule Brise l'est) → question légitime. Ça **valide la finesse du juge** (il distingue « redemander l'entité nommée » de « demander une autre entité non nommée »).

**Verdict.** **Tiering du maître INJUSTIFIÉ pour ce bug** : à la température de prod, la relance redondante est < 6 % sur small ET invariante à travers les tiers (tous 0 %) — monter en gamme n'achète rien sur cet axe (medium/large sont plus *riches* conversationnellement, mais c'est une autre dimension). Le correctif proportionné pour #62 est le **moins cher** : un ancrage few-shot dans `MASTER_SYSTEM_PROMPT` (« ne redemande jamais une info présente dans le message »), univers prompt-only. Limite assumée : 0/50 ≠ « jamais » (le dogfood prouve > 0) ; le harness ne pourra pas *prouver* le few-shot (le bug est déjà sous son plancher de détection à temp 0.1) — pour mesurer le fix il faudrait une variante **stress en température** qui fait remonter la queue. Plomberie tiering (`model` threadé) **conservée** : utile pour #49 quand un bucket le justifiera (pas celui-ci). Réf. [[project_model_tiering]].

**Pointeurs** : `evals/atelier/master_ab.py` (harness + juge + heuristique + backoff), `src/felix/core/agent.py` (`create_core_agent(model=…)`), `src/felix/atelier/agent.py` (`build_master_agent(choice, model=…)`), `justfile` (`ab-master`), `tests/unit/test_conductor.py` (univers Hector/Brise/Talou/Vorn/Roche-Pâle dans la loi anti-leakage). Log brut : `/tmp/ab_master_full.log`.

## Human-in-the-loop : l'auteur corrige/supprime depuis l'UI, et le LLM le SAIT (issue #61) — 2026-06-10

**Le besoin (utilisateur)** : quand le LLM se trompe (perso inventé #56, bataille-groupe #59, relation hallucinée), la seule « correction » possible était de raconter la correction dans le chat en espérant que l'extraction la comprenne. Désormais **chaque carte du chat et chaque ligne de la fiche portent ✎/🗑** — et l'action manuelle **entre dans le contexte du LLM**, sinon l'entité supprimée renaissait au tour suivant (elle vit encore dans l'historique threadé du maître).

**Mécanique (même recette neuro-symbolique que le working set — posé par du code, jamais par le modèle).** (1) **Routes d'édition** : `DELETE /api/entities/{id}` (DETACH DELETE ; un événement supprimé **recoud la chaîne NEXT** p→e→n ⇒ p→n), `PATCH /api/entities/{id}` (rename + set/remove de props), `DELETE .../relations/{rel_type}/{other_id}` (orientée). Le rename passe par **`rename_or_merge` extrait au niveau driver** (core/graph.py) et partagé tool↔route : éditer un nom depuis l'UI a EXACTEMENT le même effet que le tool `rename_entity` (id migré, fusion sur collision, relations/événements conservés). (2) **Tombstones `:UserEdit {kind, name, detail, ts, notified}`** posés par chaque route d'édition — hors du graphe `:GenEntity`, invisibles des tools. (3) **Injection en code** : le bloc « l'auteur a corrigé la bible À LA MAIN […] ne recrée pas, garde les valeurs corrigées » est préfixé au prompt des **extracteurs à CHAQUE tour d'extraction** tant que le tombstone vit (TTL `FLX_USER_EDITS_TTL_MINUTES`=240, borne 12), et au message du **maître UNE FOIS** (flag `notified` — le fil threadé retient, répéter ne ferait que gonfler l'historique). (4) **Les cartes portent leur cible** : `ToolCard.entity_id` (fiche/événement) ou `ToolCard.relation` (from/to/type) — sans ça, aucun bouton possible ; le lien « Voir la fiche » pointe enfin sur LA fiche. (5) **Front** : ✎ inline + 🗑 à double clic (« Confirmer ? ») sur les cartes (état « supprimé par toi » barré) ; sur la fiche : rename du titre, correction/suppression de prop, suppression de relation/événement/fiche.

**Eval-first** : suite unitaire `UserEdits` 8 tests rouges d'abord (75/75 ensuite), et **nouveau harness `just e2e-edits`** (vraie route SSE + vraies routes d'édition, univers neuf Ottra/Vels/Doran ajouté à la loi anti-leakage) : **10/10 vert** — dont LE critère de done : créer, supprimer via l'API, beat voisin → **0 résurrection** (et l'écriture continue : Doran capté) ; rename Vels→Velsgarde puis tour qui re-mentionne « Vels » → **0 doublon** (résolution par nom + bloc). Premier run : 3 rouges de harness (Small baptise « la citadelle de Vels », « capitaine Doran ») → checks passés en résolution par fragment, comme l'app.

**Limites/notes** : la passe entités a crashé une fois sur des args d'outil malformés (transient connu, passe best-effort, tour non bloqué) ; le tour 3 a réifié « un incendie éclate au marché de Vels » en entité (sur-extraction connue, #56/#46 — PAS ce chantier) ; le TTL borne la consigne « ne recrée pas » : passé 4 h, l'auteur peut réintroduire le même nom sans friction.

## Session utilisateur réelle (le fils) : les gardes au write se font contourner EN SÉRIE — 2026-06-10

Première session par un utilisateur non-développeur (10h23-10h26, « Ventilateur », jeune chevalier, plaines d'Aragorn, orcs, Nîmes) — décryptée dans les traces Logfire. **La trouvaille (issue #59, P1)** : le relieur veut matérialiser « la bataille de ventilateur » (déjà bien chroniquée en events #11/#13 par le chroniqueur) → `add_entity(type=evenement)` **refusé** (type réservé ✓) → le message d'erreur suggère « garde X pour une vraie entité (personnage, lieu, objet) » → il retente en **`groupe`** → accepté → puis `INVOLVES` refusé (hors vocab ✓) → rabattu sur **`Ventilateur FIGHTS la bataille de ventilateur`**. Une bataille est devenue un groupe qu'on combat. Même pattern sur « plaines remplies de roche volcanique » : `update_entity(description)` refusé (garde anti-écrasement ✓, qui protégeait… une description placeholder) → contourné en entité `roche volcanique [objet]` + **`plaines PART_OF roche volcanique`** (inversée) + un event-état. **Leçon : chaque garde marche, leur COMPOSITION échoue — les refus pédagogiques (« utilise plutôt X ») guident Small vers le premier trou, et la bêtise finit en canon.** Piste : refus sémantiques TERMINAUX (« n'enregistre pas, ne recrée sous aucun autre type »), voire mémoire des refus du tour en code.

Aussi : **hallucination relationnelle** `orcs LOCATED_AT Nîmes` (seul Ventilateur y rentre) ; **contamination cross-histoires confirmée en réel** (la session jouée sans wipe après mon dogfood → bible pétrole+LOTR mélangées, le working set montrait Vigdis/Harlan à une session chevaliers) → issue #60 (scoping projet, le serpent de mer d'Alger enfin filé). Positifs : les events du fils étaient propres (#11-13), les gardes prises isolément ont refusé 9 écritures fausses, et `rename`/dédup events ont tenu.

## Dogfood « plateforme pétrolière » (post-gate) : le ROUTAGE est réglé (11/11), le front de bataille se déplace sur l'EXTRACTION — 2026-06-10

Dogfood à la volée juste après le gate (#43), vraie route SSE, base vidée, univers neuf (huis clos polar/fantasy : Vigdis, Harlan, l'Aumônier, mer du Nord). **11 tours, 11 décisions de routage correctes — dont LE scénario tueur d'hier** : tour 1 hedgé (« j'ai une vague idée mais c'est pas encore clair... un huis clos sur une plateforme pétrolière ? je sais pas trop ») → routé ; tour 2 bavardage → 0 écriture SANS contaminer le tour 3 (l'ornière ne s'amorce plus) ; questions de lecture → 0 écriture, réponses exactes en 1,6-1,8 s ; bavardages 0,6-0,9 s. **Double rename au baptême différé ✓✓** (« l'ingénieure s'appelle Vigdis. et le chef, on va dire Harlan » → 2 cartes « Entité renommée », 0 doublon). Correction `motif` propre. Graphe final : 6 entités réelles, 0 doublon, 0 placeholder. Le chantier routage est CLOS.

**MAIS le gate déplace le front : MAJEUR #1 (issue #56) — un tour vague routé fait REMPLIR UN GABARIT à l'extracteur.** Le tour 1 d'ambiance (aucun personnage dedans) a produit **« Maximilien [personnage] age='35', background='Ancien militaire', traits='Déterminé, méfiant' » — inventé de toutes pièces** ; et au résumé du tour 11, la lecture le recycle en « Maximilien, le technicien décédé » → **l'inventé devient canon**. Avant le gate, ces tours d'ébauche n'étaient pas routés (le bug en cachait un autre). Piste la plus propre : passer `RouteDecision.fait` (déjà calculé par le gate) aux extracteurs — « extrais CE fait, rien d'autre ».

**MAJEUR #2 (issue #57) — la VICTIME du polar n'a jamais eu de fiche.** « elle vient remplacer un technicien qui est mort la semaine d'avant » : le technicien traverse 3 tours (mort suspecte, caisson, spirale gravée) sans nœud → l'event « grave une spirale » flotte SANS participant, et la lecture confabule (cf. #1). Pareil pour l'Aumônier : mentionné tour 6 (en complément), fiche seulement tour 9 (quand il devient SUJET). La passe 1 ne crée que les agents en position de sujet.

**NOUVEAU (issue #58)** : `update_entity` a posé `traits='observateur'` **sur un nœud ÉVÉNEMENT** (« L'Aumônier dit que la boue murmure » matché par nom quand le perso n'existait pas) → exclure les events de la résolution d'écriture, comme le working set le fait déjà.

**Confirmés (commentés sur les issues)** : #44 flashback tel quel (« trois jours AVANT […] c'est un flashback » explicite → event #8 en FIN de chronologie, rien pour corriger) ; #45 relieur ré-émet l'existant (2 LOCATED_AT dupliqués) ET sous-relie (aucune relation Vigdis↔Harlan après 9 tours) ; #46 chroniqueur états→events + **variante neuve : il RE-CHRONIQUE au tour de baptême** (« l'ingénieure s'appelle Vigdis » → 2 events paraphrasés des events du tour précédent sous les nouveaux noms — un baptême ne raconte rien, et la dédup exacte est aveugle aux paraphrases).

**Lecture d'ensemble** : la passe 0 (gate+maître) tient ; les frustrations restantes sont TOUTES dans les passes 1-3 (extraction). L'ordre d'attaque que ça suggère : #56 (le canon pollué est le pire pour l'auteur) → #57 (recall entités) → #46/#45 (bruit).

## Gate de routage STATELESS : l'ornière d'auto-imitation est morte par construction (issue #43) — 2026-06-10

**Le fix structurel de l'ornière du dogfood (section ci-dessous) : la DÉCISION d'extraire est sortie du fil threadé.** Avant : le maître décidait (outil-signal `noter_le_passage`) dans le MÊME appel threadé que sa réponse → la décision voyait ses décisions passées, et Small imitait son propre pattern plus fort que le system prompt (un tour 1 hedgé non routé rendait la session entière muette). Après : un **gate stateless** (`build_gate_agent`, `RouteDecision {fait, noter}` en structured output, AUCUN outil, AUCUN historique — il ne voit QUE le message du tour) décide ; le maître devient **purement conversationnel** (lecture seule, prompt allégé — le gros bloc de routage SORT du fil threadé, payé une fois par tour dans un appel court au lieu d'être re-traîné à chaque tour). Chaque décision est un tour 1 : **l'ornière est impossible par construction**, plus rien à prompter contre.

**Détails de design.** (1) **Reason-first dans le schéma** : `fait` (le fait formulé) AVANT `noter` (le booléen) — l'ordre des champs est l'ordre de génération, tester le contraire ferait trancher Small à l'aveugle ; garanti par un test structurel (`model_fields`). (2) **Few-shot « hedge + contenu = contenu »** dans le gate (univers prompt-only Sel/Mirko/Vellone) : « ignore les marqueurs de doute ; une fois le doute retiré, s'il reste une ébauche d'histoire → noter ». (3) **Parallélisme** : la route lance le gate en `asyncio.create_task` PENDANT le stream du maître → latence masquée, verdict attendu juste avant le dispatch des extracteurs. (4) **Fail-closed** : gate en erreur ⇒ pas d'extraction (l'invariant n°1 reste 0 hallucination ; le fait peut être redonné). (5) Le gate est UNIQUE (pas par profil) — décider si un message pose un fait ne dépend pas du domaine. (6) `noter_le_passage` supprimé partout (l'outil-signal n'a plus de raison d'être).

**Eval-first, A/B propre sur `just e2e-conductor` étendu d'une 2e session « ornière »** (tour 1 hedgé+contenu, univers neuf Vada/Tilio/Sorne, recall PLEIN exigé — la session mêlée garde sa tolérance n-1) : **pré-fix ROUGE exactement sur le bug** (tour 1 hedgé : 0 écriture ; recall 2/3 ; session mêlée verte) → **post-fix 9/9 VERT** (tour 1 hedgé routé avec 7 écritures, recall 3/3, 0 hallucination sur les 2 sessions, 0 placeholder). Sonde gate isolée `/tmp/probe_gate.py` : **8/8** (salutation/question/remerciement/« je sais pas » non routés ; contenu net, hedge+contenu, baptême bref, correction routés). Non-régression : `just e2e-atelier` 5/5 invariants, unit **67/67**, ruff/lint/typecheck verts.

**Garde-fous neuro-symboliques autour (les 2 autres pistes du dogfood).** (a) **Bannière « session muette »** (pur code, front) : 3 tours d'auteur sans AUCUNE carte d'écriture et bible jamais remplie → bannière sobre au-dessus du composer (`useAtelier.ts` compte, `chat.vue` affiche) — le filet pour les modes de défaillance qu'on n'a pas encore vus ; le compteur repart dès que la bible bouge. (b) **Test anti-leakage STRUCTUREL** (`test_no_test_universe_leaks_into_prompts`) : les univers des tests de routage (Nora/Castan, Vada/Sorne, Adator/Alikazeth…) ne peuvent plus entrer dans `GATE_SYSTEM_PROMPT`/`MASTER_SYSTEM_PROMPT` sans casser les unit — [[feedback_prompt_test_leakage]] devient une loi vérifiée, plus une discipline. (Match mot entier : la v1 naïve attrapait « Veil » dans « sur**veil**le ».)

**⚠ Découverte infra au passage (issue #55)** : Mistral gèle aléatoirement un appel (~1/40 ce matin, 2 spans Logfire à ~900 s pour des appels normalement < 1 s) et `build_model` ne fixe AUCUN timeout client → chaque gel fige une session/un run 15 min sur le défaut du SDK. Quick-win : timeout ~60 s (p99 réel < 7 s).

**Reste du chantier #43** : rien — fermée avec cette section. Les frustrations dogfood restantes vivent dans #44 (flashback), #45 (relieur effet-menu), #46 (chroniqueur états), #47 (props inventées).

## Dogfood « cirque années 30 » (2 sessions) : l'ORNIÈRE d'auto-imitation du maître, et le reste de la chaîne validé live — 2026-06-10

Dogfood à la volée (`/tmp/felix_chat.py`, base vidée, univers neuf jamais utilisé — règle [[feedback_prompt_test_leakage]]) : polar dans un cirque itinérant, fin des années 30. **Deux sessions aux destins opposés, et c'est la trouvaille.**

**MAJEUR #1 — L'ornière d'auto-imitation du maître (la session 1 entière perdue).** Tour 1 ouvert par un hedge d'auteur (« J'ai une idée de polar mais c'est encore flou : … » + contenu réel) → non routé. Ensuite **PLUS AUCUN tour ne route** — 5 tours de contenu (baptêmes « Luce »/« Brando » compris), **0 écriture**, réponses en 0,6-0,9 s. Au nudge « tu enregistres ? la bible a l'air vide » → Felix **ment** : « Oui, je note les éléments » (le say/do au niveau du MAÎTRE, pire que tout). **Preuve d'ornière** : le MÊME message tour-3, rejoué en historique VIDE → routé (4 phases). Donc : Small **imite son propre pattern en contexte** (réponse-sans-tool-call) plus fort que le system prompt ; un tour 1 raté condamne la session ; inversement la session 2 (tour 1 net → routé) a routé tous ses tours de contenu, Y COMPRIS un tour « entité sans nom » (la classe qui semblait fautive). Ça réconcilie les mesures contradictoires du journal (sonde isolée 92 % vs « bien meilleur en flux threadé » du e2e conductor : tout dépend du PRÉCÉDENT). **La sonde actuelle (history=[]) ne capture pas cette classe** → il faut une sonde/eval THREADÉE qui démarre par un tour hedgé. **Pistes de fix** : (a) sortir la DÉCISION de routage du fil threadé (gate stateless par tour — le « gate d'intention » historique, la décision ne voit pas les décisions passées) ; (b) few-shot « hedge + contenu = contenu » ; (c) garde-fou produit : bible vide après N tours affirmatifs → bannière/relance.

**MAJEUR #2 — La chronologie est infligeable (flashback).** Tour 5 : « le lendemain soir, le lanceur de couteaux retrouve Luce » → **l'alerte temporelle tombe, en live, avec un message propre** (« ne peut pas glisser un mot à Luce après être mort ») — le checker v1 marche en conditions réelles ✓. MAIS tour 6 : « c'était la veille de sa mort en fait, je raconte dans le désordre » → **non routé** (traité en bavardage), et de toute façon **aucun outil ne sait réordonner/insérer un événement ou marquer un flash-back** → l'event #6 reste mal ordonné, l'alerte (juste à l'émission) devient un faux positif permanent. Raconter dans le désordre est LE mode naturel d'un auteur. → v1-suite (3) du HANDOFF + un outil d'édition d'ordre (insert-avant / marquer flashback).

**MOYEN — Relieur en roue libre (cartes seulement).** Tour 4 : **14 cartes relations dont ~tout en double dans le même tour** (`OWNS ×2`, `LOCATED_AT ×2`, `WITNESSES ×2`, `MEMBER_OF ×4`…). En BASE : 0 doublon (MERGE dédup) — c'est du bruit d'UI et des appels gâchés. **Hypothèse à tester : effet-menu du bloc `[CONTEXTE]`** (il voit la liste des entités récentes → il relie tout, re-émet l'existant). Pistes : inclure les relations EXISTANTES dans son contexte (« celles-ci existent, n'émets que les NOUVELLES »), ou dédup des cartes par tour côté route.

**MOYEN — Chroniqueur : états/méta en événements (confirmé ×2).** « ne croit pas à l'accident » (#3, état mental), « était la maîtresse » (#4, état durable), « Brando n'est pas le directeur » (#5, correction méta) → 3 events sur 6 ne sont pas des actions. La règle « pas d'état mental » du prompt ne tient pas sur Small.

**MOYEN — Props inventées / absurdes (passe 1).** `camp='neutre'`, `age='adulte'`, `ambiance='rurale'` posés SANS que l'auteur l'affirme (violation « n'invente aucun fait », version sournoise : du remplissage de gabarit) ; clé **auto-référente** `relation_avec_lanceur_de_couteaux='amant'` sur le lanceur LUI-MÊME ; nom recopié dans `traits` (`traits='lanceur de couteaux'`, le pattern « mage noir »). Et « maîtresse » fini en prop faute de type de relation amoureuse dans le vocab (LOVES absent — trou de vocab, même famille que (1) du HANDOFF).

**POSITIF (tout le reste de la chaîne tient en live).** **Rename au baptême ✓** (« La trapéziste s'appelle Luce » → carte « Entité renommée », 1 seul nœud, le bug Adator est mort en réel) ; correction explicite ✓ (`role='régisseur'` remplacé proprement) ; **alerte temporelle live ✓** ; lecture bible exacte ✓ (« fais-moi le point sur Luce » → résumé fidèle, 0 écriture, 1,5 s) ; base finale propre (13 entités, 23 relations, 0 doublon) ; tours conversationnels ~1 s (gain conducteur), tours de contenu 10-24 s. **UX mineur** : Felix ne réagit pas à l'alerte dans SA réponse (il enchaîne comme si de rien) ; en session 1 il posait 2 questions collées (« l'accident ?Et qui »).

**Priorités qui sortent** : (1) ornière du maître (gate stateless ou équivalent) — c'est LE risque produit, silencieux et total ; (2) flashback/ordre des événements ; (3) effet-menu du relieur (lié à mon bloc [CONTEXTE] du jour — à instrumenter) ; (4) chroniqueur états→events + props inventées (bornés modèle, à few-shoter ou parker).

## Recall du maître : décision AVANT le texte + few-shot baptême (53 % → 92 %) ; règle anti-leakage — 2026-06-10

**Baseline catastrophique mesurée** (sonde `/tmp/probe_routing.py` étendue d'une classe « baptême ») : **53 %** de précision de routage, 6 faux négatifs — dont des tours riches. Le symptôme décisif : le maître **dit « Noté, Veil est l'homme de main »** sans appeler l'outil, et va jusqu'à écrire `Noter_le_passage("…")` **en texte** dans sa réponse. **Cause** : le prompt ordonnait « 1. Réponds, 2. Décide » → Small répond et S'ARRÊTE (`finish_reason: stop`) sans jamais atteindre l'étape 2. C'est le principe **reason-first** ([[reference_prompting_research]]) violé dans l'ordre des étapes.

**Fix (pur prompt, `MASTER_SYSTEM_PROMPT`)** : (1) **décision et appel d'outil AVANT la réponse** ; (2) règle + few-shot **baptême** (« un NOM donné à une entité qu'on suivait sans nom ») ; (3) anti-patterns explicites : « noter_le_passage est un APPEL D'OUTIL, jamais du texte ; ne dis JAMAIS "noté" sans l'avoir réellement appelé ». **Résultat : 92 % (12/13), 0 faux positif (maintenu), baptêmes 3/3** — y compris « le mage noir se nomme Alikazeth » alors que l'exemple du prompt est « le contremaître se nomme Drass » (généralisation, pas récitation). Seul raté restant : « À l'aube sur le quai, Veil surveille Nora de loin » (action descriptive brève, history vide) — candidate au tiering du seul maître ([[project_model_tiering]]).

**Règle posée par l'utilisateur (mémorisée `feedback_prompt_test_leakage`) : les exemples des prompts ne servent JAMAIS dans les tests/sondes/evals.** Constat en le corrigeant : la sonde d'hier rejouait « Nora débarque à Port-Vendres… » quasi VERBATIM depuis le few-shot du maître (métrique partiellement triviale), et j'avais d'abord mis « le mage noir se nomme Alikazeth » dans le prompt — l'exact cas de la sonde ET de l'eval. **Fix** : univers d'illustration **prompt-only** (Sel/Mirko/Vellone, cité minière) pour les few-shots du maître ; bloc `[CONTEXTE]` du working set dé-exemplifié (l'instruction sans exemple concret) ; tours « sans contenu » de la sonde reformulés. Les 92 % ci-dessus sont mesurés APRÈS dé-corrélation.

**Non-régression** : `just e2e-conductor` tout vert (0 hallucination / 5 tours sans contenu, 0 placeholder, recall OK) ; `bapteme_differe` vert au rejeu (1/1, graphe final = Adator + Alikazeth seuls) — avec **1 échec de variance** entre deux verts (3 runs post-fix : vert/rouge/vert ; cas à passer en ForEach/multi-run si ça flake) ; unit 65/65 ; ruff propre.

## Working set d'entités (récence) : fin du « baptême différé » + trou LOCATED_AT groupe — 2026-06-10

**Bug live (session Adator, lu directement dans les traces via le MCP Logfire)** : l'auteur décrit « un mage noir », puis « le mage noir se nomme "Alikazeth" » → fiche `Alikazeth` NEUVE au lieu d'un rename, doublon permanent. La trace montre DEUX causes en interaction : (1) le tour de baptême n'a **pas été routé** par le maître (pas de `noter_le_passage`, le recall connu sur tours brefs — pire tour possible à rater) ; (2) au tour suivant, la passe 1 fait `describe_schema → add_entity Alikazeth` **sans jamais relire la base** (`find_entity`). **Effet de bord du chef d'orchestre identifié** : l'historique threadé = le fil du maître → la passe 1 ne voit PLUS ses propres écritures passées (avant, son `add_entity("Mage Noir")` du tour N-2 était dans l'historique) ; elle dépend à 100 % de la relecture base, que Small ne fait pas spontanément malgré la règle 1.

**Fix neuro-symbolique (design utilisateur : « cache » borné par récence, pas un dump de la base)** :
- `last_touched` (ms epoch, `timestamp()` Neo4j) tamponné **EN CODE** par les tools — toutes les écritures (add/update/rename/add_relation/add_event-participants) ET la lecture résolue (`find_entity`) — `core/graph.py touch_entities`.
- `recent_entities(driver, limit)` : les N entités non-événement les plus récemment touchées (coalesce 0 — Neo4j trie les null en PREMIER en DESC ; les nœuds d'avant-tampon restent listés en queue). **La borne scope naturellement sur l'histoire en cours** (à 400 entités, les persos d'un autre pan du monde ne remontent jamais tant qu'on n'y touche pas — l'exemple utilisateur : travailler l'ép. 7 de Star Wars ne charge pas Clone Wars).
- `render_recent_block` (PUR, testé) : bloc `[CONTEXTE, pas du récit — entités DÉJÀ en base …]` + consigne rename/update, préfixé **en code** au prompt des passes **entités + relieur** (route ET harness eval, qui shunte la route) ; maître et chroniqueur gardent le message nu. `FLX_RECENT_ENTITIES_LIMIT` (défaut 30).

**Eval-first, 3 niveaux** : unit 5 cas `test_recent_entities.py` (fail-first constaté) ; sonde `/tmp/probe_recent.py` 9 checks sans LLM (stamping par chaque tool, ordre de récence, borne, événements exclus, rename en tête, nœud pré-tampon en queue) ; eval commitée `bapteme_differe` (2 beats : mage noir anonyme → baptême) avec **A/B sur la même build** : `FLX_RECENT_ENTITIES_LIMIT=0` (injection coupée) → `char_count=2` (le doublon Adator reproduit), défaut 30 → **1/1 vert** (`char_count=1`, Alikazeth unique — **Small UTILISE rename_entity quand il voit la base**), et tour moins cher (41,5k tok in vs 54k : rename < création+relations). **Non-régression** : cas seedés 3/3 (`no_duplicate_existing`, `create_among_existing`, `read_list` — le bloc ne fait ni écrire sur une question ni dupliquer), unit **65/65**, `just e2e-atelier` Le Nadir **5 invariants verts, 0 tour en erreur** (un premier run mangé par un 504 Mistral de 905 s — transient, repassé en 8 s).

**Bonus vocab (trouvé par la sonde)** : `Orques [groupe] LOCATED_AT Adator` était **refusé au write** — `LOCATED_AT.subjects` ne contenait pas `groupe` (reliquat d'avant l'ajout du type groupe : on avait étendu MEMBER_OF/ALLIED_WITH/FIGHTS, pas LOCATED_AT). Donc dans la session Adator live, ces relations avaient été silencieusement rejetées. Fix une ligne (`profile.py` + exemple « le groupe vit à »), test rouge→vert.

**Reste / parké** : (a) le **recall du maître sur les baptêmes** — le tour « X se nomme Y » n'avait pas été routé ; few-shot dédié dans le prompt du maître et/ou tiering du seul maître ([[project_model_tiering]]) ; le working set ne répare que le tour d'après ; (b) harness : l'historique threadé des evals accumule les blocs `[CONTEXTE]` des beats passés (la route, elle, threade le fil du maître, sans blocs) — mineur, contexte balisé ; (c) la fiche `Mage Noir` d'origine illustrait encore la modélisation pauvre de Small (`background='mage', traits='noir'` — le nom découpé en props), chantier [[project_modeling_quality]].
