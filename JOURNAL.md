...
## ▶ HANDOFF (reprendre ici) — modèle événementiel + 2 quick-wins — 2026-06-08

**État** : tout le travail relations (vocab EN + 2e passe relieur) est sur le disque, **non commité**, evals atelier 13/13 vertes. Test grandeur nature « Le Nadir » (scénario sci-fi noir, ~12 beats) passé en live → 3 verrues identifiées, par priorité :

**(1) ÉVÉNEMENTS écrasés dans des props `action`/`etat` — LE gros sujet (modélisation, état vs événement).**
- Symptôme : chaque beat fait `update_entity(Vance, action='…')` qui **écrase** le précédent (`tire sur les consoles` → `verrouille l'écoutille` → `traîne la mercenaire`). Idem Capitaine/intrus. La **chronologie est perdue** : il ne reste que le dernier geste. Le type `evenement` du profil n'est pas utilisé.
- **Design retenu** (à implémenter) : un node `:GenEntity {entity_type:"evenement", resume, beat/ordre}` par action ; chaînage par prop ordinale `ordre` (tri robuste) **+** relation canonique `NEXT` (event N→N+1) ; participants via `event —INVOLVES→ perso` et `event —LOCATED_AT→ lieu`. Principe **état vs événement** : sur le perso = durable (background/traits/arc/statut vivant-mort) ; en event = ce qui arrive à un instant. Bonus checker : « mort puis agit » devient vérifiable par l'ordre des events. Réf. vision `docs/vision_produit_copilote_scenario.md` (modèle état/événement + ontologie hybride).
- **Questions ouvertes** : qui crée les events — l'agent d'entités, le relieur, ou une **3e passe « chroniqueur » dédiée** ? ; comment l'agent tranche state-vs-event (règle de prompt + few-shot) ? ; faut-il ajouter `evenement` aux types poussés + `NEXT`/`INVOLVES` au `relation_vocabulary` ? ; eval-first : un cas qui vérifie qu'un beat d'action crée un event (pas un overwrite de prop) et que l'ordre est préservé.

**(2) Trous de vocab → dérive qui revient (quick-win, ~20 min, eval-first).**
- Vu sur Le Nadir : `Capitaine TRANSPORTE prototype`, `Vance AIDE intrus` (FR !), `Capitaine ORDERS Vance` (EN mais hors des 12). → ajouter `COMMANDS`, `HELPS`, `TRANSPORTS` (+ peut-être `INVOLVES`, `NEXT` pour (1)) à `SCENARIO_PROFILE.relation_vocabulary`, et une consigne « types de relation TOUJOURS en anglais ; si aucun canonique ne colle, rabats-toi sur le plus proche avant d'inventer ». Re-mesurer `rel_vocab_coverage` ; promouvoir son seuil en gate (`min_coverage` ~0.8) une fois stable.

**(3) Le relieur (full tools) re-fait des `update_entity` (cosmétique, à border).**
- Vu : cartes doublées (`Vance action=…`, `intrus action=…`) émises par le relieur *avant* ses relations → il refait le travail d'entités. Le filet « add_entity pour backfill » est devenu « redo ». Resserrer la discipline `RELATION_SYSTEM_PROMPT` (priorité absolue add_relation, ne PAS ré-update une entité existante) ; ne pas retirer update_entity des outils (sinon crash sur outil manquant, cf. piège déjà traversé).

**Pointeurs** : noyau `src/felix/core/` (agent.py `SYSTEM_PROMPT`/`RELATION_SYSTEM_PROMPT`/`create_core_agent`, profile.py `relation_vocabulary`, tools.py, check.py) ; relieur `src/felix/atelier/agent.py` ; route 2e passe `src/felix/api/routes/atelier.py` ; evals `evals/atelier/{task,dataset,evaluators}.py` (cas `roue_de_sang` multi-beats, evaluators `relations_present`/`rel_vocab_coverage`/`entity_unique`) ; banc de mesure `/tmp/measure_relations.py` (`TWO_PASS`, retry 5xx). Modèle pinné `mistral-small-2506` (cf. `.env`). Tiering modèle Large/Small parké (cf. mémoire `project_model_tiering`).
