...
## ▶ HANDOFF (reprendre ici) — front 100 % schemaless FAIT ; quick-wins vocab + relieur en attente — 2026-06-09
...
**(2) Le relieur (full tools) re-fait des `update_entity` (cosmétique, à border).**
- Cartes doublées émises par le relieur *avant* ses relations → il refait le travail d'entités. Resserrer la discipline `RELATION_SYSTEM_PROMPT` (priorité absolue add_relation, ne PAS ré-update) ; ne pas retirer update_entity des outils (sinon crash sur outil manquant, piège déjà traversé).
...
**(4) Relations TYPÉES + vocab DUR (idée utilisateur 2026-06-09, à faire avec la qualité de modélisation).**
- Aujourd'hui : une seule relation Cypher `:REL` + propriété `rel_type` (choisi pour rester Cypher pur sans APOC), et le `relation_vocabulary` du profil n'est qu'une **suggestion molle** du prompt → le LLM **invente** (`INTERROGATES`, `AIDE` en FR, `MEMBER_OF → un objet`). Vu en direct sur l'histoire « Alger 1957 ».
- **Idée** : faire du `relation_vocabulary` une **loi**. Faisable **sans APOC** : valider `rel_type` contre la **liste blanche fermée du profil**, puis **interpoler** le type dans la requête (sûr car enum contrôlé par le code, pas d'injection) → vraies relations typées `:KNOWS`/`:OWNS` dans Neo4j Browser, et `all_relations`/`neighborhood` plus simples (`type(r)`). Le **vrai gain** est l'**enforcement au niveau écriture** : on interdit mécaniquement les types hors-vocab → règle une partie des bugs de modélisation.
- **Contraintes actées avec l'utilisateur** : (a) **vocab PARAMÉTRABLE comme le template** (par profil/domaine, PAS un set mondial figé — un polar ≠ un livre de cuisine) ; (b) **seules les relations se ferment, les entités restent 100 % schemaless** (types + props libres). Ne règle pas tout : le typage **domaine/portée** (`MEMBER_OF` exige une organisation, pas un objet ; `OWNS` dans le bon sens) reste à border ensuite — créer aussi les entités « organisation » manquantes (le FLN jamais modélisé → l'appartenance se greffe sur le tract). Refactor borné : `core/graph.py` (writer + lecteurs + machinerie events `INVOLVES`/`NEXT`) + tool `add_relation` + un poil de front (lire `type(r)`). Eval-first.
- **Bugs relationnels confirmés sur le run « Alger 1957 » (12 beats, graphe propre)** : `tract PART_OF tract` (self-loop), `cave LOCATED_AT registre` (lieu→objet), `Laurent TARGETS tract` (cible = objet possédé), `Dubois WITNESSES école` (cible = lieu), `école PART_OF cave` (direction inversée), churn (mêmes relations re-add chaque beat, MERGE les sauve). Le check en a attrapé 2/6 → enforcement au write > détection.
