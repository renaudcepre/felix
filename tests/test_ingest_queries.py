...
from felix.graph import repository as queries
...
async def test_create_and_list_issues(seeded_driver: AsyncDriver) -> None:
    issue = _make_issue()
    await queries.create_issue(seeded_driver, issue)
    rows = await queries.list_issues(seeded_driver)
    assert any(r["id"] == issue["id"] for r in rows)


async def test_list_issues_filter_type(seeded_driver: AsyncDriver) -> None:
    await queries.create_issue(seeded_driver, _make_issue(type="character_contradiction"))
    await queries.create_issue(seeded_driver, _make_issue(type="missing_info"))
    rows = await queries.list_issues(seeded_driver, type="missing_info")
    assert all(r["type"] == "missing_info" for r in rows)


async def test_list_issues_filter_severity(seeded_driver: AsyncDriver) -> None:
    await queries.create_issue(seeded_driver, _make_issue(severity="error"))
    await queries.create_issue(seeded_driver, _make_issue(severity="warning"))
    rows = await queries.list_issues(seeded_driver, severity="error")
    assert all(r["severity"] == "error" for r in rows)


async def test_list_issues_filter_resolved(seeded_driver: AsyncDriver) -> None:
    await queries.create_issue(seeded_driver, _make_issue(resolved=False))
    await queries.create_issue(seeded_driver, _make_issue(resolved=True))
    rows = await queries.list_issues(seeded_driver, resolved=False)
...
async def test_update_issue_resolved(seeded_driver: AsyncDriver) -> None:
    issue = _make_issue()
    await queries.create_issue(seeded_driver, issue)
    ok = await queries.update_issue_resolved(seeded_driver, issue["id"], True)
    assert ok is True
    rows = await queries.list_issues(seeded_driver, resolved=True)
    assert any(r["id"] == issue["id"] for r in rows)


async def test_update_issue_resolved_not_found(seeded_driver: AsyncDriver) -> None:
    ok = await queries.update_issue_resolved(seeded_driver, "nonexistent", True)
    assert ok is False
...
async def test_delete_issues_for_scenes(seeded_driver: AsyncDriver) -> None:
    ...
    await queries.create_issue(seeded_driver, i1)
    await queries.create_issue(seeded_driver, i2)
    await queries.create_issue(seeded_driver, i3)
    await queries.delete_issues_for_scenes(seeded_driver, ["scene-a", "scene-b"])
    rows = await queries.list_issues(seeded_driver)
...
async def test_upsert_and_list_scenes(seeded_driver: AsyncDriver) -> None:
    ...
    await queries.upsert_scene(seeded_driver, scene)
    rows = await queries.list_scenes(seeded_driver)
...
async def test_upsert_character_minimal_ignore(seeded_driver: AsyncDriver) -> None:
    ...
    row = await queries.get_character_profile(seeded_driver, "marie-dupont")
...
async def test_upsert_character_minimal_new(seeded_driver: AsyncDriver) -> None:
    ...
    row = await queries.get_character_profile(seeded_driver, "new-char")
    assert row is not None
...
async def test_upsert_location_minimal_ignore(seeded_driver: AsyncDriver) -> None:
    ...
    detail = await queries.get_location_detail(seeded_driver, "lyon-safe-house")
...
async def test_upsert_timeline_event(seeded_driver: AsyncDriver) -> None:
    ...
    await queries.upsert_timeline_event(seeded_driver, evt)
    rows = await queries.get_timeline_rows(seeded_driver, date_from="1942-04-01", date_to="1942-04-01")
    assert len(rows) == 1
...
async def test_upsert_character_event(seeded_driver: AsyncDriver) -> None:
    ...
    await queries.upsert_timeline_event(seeded_driver, evt)
    await queries.upsert_character_event(seeded_driver, "marie-dupont", "evt-ce-test", "participant")
    rows = await queries.get_timeline_rows(seeded_driver, date_from="1942-04-01", date_to="1942-04-01")
