...
async def persist_source_pages(
    driver: AsyncDriver, document_id: str, pages: list[str], *, project: str,
) -> None:
    ...
    rows = [
        {"page": i, "text": text} for i, text in enumerate(pages, start=1) if text.strip()
    ]
    if not rows:
        return
    async with driver.session() as session:
        await session.run(
            ...
            """,
            rows=rows, project=project, doc=document_id,
        )


async def source_pages_for(
    driver: AsyncDriver, entity_ids: Iterable[str], *, project: str,
